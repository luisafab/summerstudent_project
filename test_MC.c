...
int test_MC( ) {
    ...
    ROOT::RDataFrame df_MC("O2v0tableap", "data/AnalysisResults_treesAP_data.root");
    //auto cutIsRecoPDG = [](bool IsReco, int PDG) { return ((PDG == 3122 || PDG== -3122) && IsReco); };
    std::cout<<"done1"<<std::endl;
    //apply cut as filter on data frame 
    auto df = df_MC.Filter(isMC? "(fPDGCode == 3122 || fPDGCode== -3122) && fIsReco" : "fLen>0");
    std::cout<<"done2"<<std::endl;

    /*auto h0 = df.Histo1D("fPDGCode");
